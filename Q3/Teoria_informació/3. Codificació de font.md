# Font d'informació
Dispositiu que emet seqüències de símbols d'acord amb una certa probabilitat.

Aquesta pot tenir memòria (procés estocàstic) o no tenir memòria: generar símbols segons una variable aleatòria $X\in\mathcal X$.
## Codi font
codi $C_{x}\subset\set{0,1}^{*}$ amb codificació $$enc:\mathcal X\to C_{X}$$
## Codi de Shannon
Codificació de la font amb paraules de longitud $$l_{i}=\lceil-\log p_{i}\rceil$$
...
## Codificació de font binària sense memòria
...
## Codificació de fonts amb memòria
Si $C_{X^{n}}$ és un codi òptim per la variable $X^{n}$ per cada $n$ (com un procés estocàstic), aleshores $$\lim_{n\to\infty}\frac 1 n \overline l(C_{X^{n}})\to H(X)$$ on $H(X)=\lim_{n\to\infty}\frac{H(X_{1},\dots,X_{n})}{n}$ és la taxa d'entropia.

## Joc de les preguntes
Un codi de font òptim està relacionat amb el nombre mínim de preguntes binàries (y/n) que cal fer per esbrinar el resultat d'una variable aleatoria.

Quan es juga amb diverses repeticions al joc el nombre mínim de preguntes tendeix a $\overline l(C_{X})$.

L'algoritme de Huffman ens permet tenir un roadmap de les preguntes a fer per minimitzar el nombre esperat de preguntes.